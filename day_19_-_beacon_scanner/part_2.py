...
def solution(elements: list[str]) -> int:
	...
	max_scanner_manhattan_distance = 0

	orient_all(known_beacons, known_scanners, unaligned_readings)

	for x0, y0, z0 in known_scanners:
		for x1, y1, z1 in known_scanners:
			scanner_manhattan_distance = abs(x1 - x0) + abs(y1 - y0) + abs(z1 - z0)
			max_scanner_manhattan_distance = max(max_scanner_manhattan_distance, scanner_manhattan_distance)

	return max_scanner_manhattan_distance
...
def main():
	...
	print('Day 19 : Beacon Scanner - part 2')
